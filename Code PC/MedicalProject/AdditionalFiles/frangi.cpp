...
int round_Cstm(float number);
float nextafterf_Cstm(float x, float y);
...
void frangi2d_hessian(const Mat &src, Mat &Dxx, Mat &Dxy, Mat &Dyy, float sigma){
	...
	int n_kern_x = 2*round_Cstm(3*sigma) + 1;
	...
	int i=0, j=0;
	for (int x = -round_Cstm(3*sigma); x <= round_Cstm(3*sigma); x++){
		j=0;
		for (int y = -round_Cstm(3*sigma); y <= round_Cstm(3*sigma); y++){
